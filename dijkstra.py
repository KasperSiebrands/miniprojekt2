...
def dijkstra(grid, start, goal):
    terrain_costs = get_terrain_costs()  #added terrain cost from maplayout.py
    ...
    while queue: #keeps looping untill no nodes left, while better than for loop because not know left.
        ...
        neighbors = get_neighbors(current_node, grid) #neighbors of current node

        for neighbor in neighbors:
            terrain_type = grid[neighbor[1]][neighbor[0]]  #what is the terrain type?
